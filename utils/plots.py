...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    minhw = int(min(x[2]-x[0], x[3]-x[1]))
    maxhw = int(max(x[2]-x[0], x[3]-x[1]))
    # tl = line_thickness or int(max(minhw//50,1))
    tl = max(maxhw//200,1)
    # font_scale = math.floor(0.01 * min) + 1  # line/font thickness
    font_scale = np.clip(minhw/150, 0.1, 1.5)
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    cv2.rectangle(img, c1, c2, color, thickness=tl, lineType=cv2.LINE_AA)
    if label:
        t_size = cv2.getTextSize(label, 0, fontScale=font_scale, thickness=tl)[0]
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
        cv2.rectangle(img, c1, c2, color, -1, cv2.LINE_AA)  # filled
        cv2.putText(img, label, (c1[0], c1[1] - 2), 0, font_scale, [225, 255, 255], thickness=tl, lineType=cv2.LINE_AA)
...
def plot_single_label(names, labels, save_dir=Path(''), loggers=None):
    if isinstance(names, list):
        for i,name in enumerate(names[:10]):
            cur_labels = labels[labels[:,0]==i]
            if cur_labels.size>0:
                plot_single_label(name, cur_labels, save_dir=save_dir, loggers=loggers)
    else:
        # plot dataset labels
        print(f'Plotting labels for {names} ')
        c, b = labels[:, 0], labels[:, 1:].transpose()  # classes, boxes
        nc = int(c.max() + 1)  # number of classes
        colors = color_list()
        x = pd.DataFrame(b.transpose(), columns=['x', 'y', 'width', 'height'])
        ...
        # seaborn correlogram
        sns.pairplot(x, corner=True, diag_kind='auto', kind='hist', diag_kws=dict(bins=50), plot_kws=dict(pmax=0.9))
        plt.savefig(save_dir / f'{names}_correlogram_label.jpg', dpi=200)
        plt.close()
        ...
        plt.savefig(save_dir / f'{names}_label.jpg', dpi=200)
        matplotlib.use('Agg')
        plt.close()
...
def plot_dataset_for_all_classes(dataset, names, save_dir=''):
