...
def train(hyp, opt, device, tb_writer=None):
    ...
    dataloader, dataset = create_dataloader(train_path, imgsz, batch_size, gs, opt,
                                            ...
                                            image_weights=opt.image_weights, quad=opt.quad, prefix=colorstr('train: '), cut_paste=opt.cut_paste)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if wandb_logger.wandb:
                ...
                for i, _name in enumerate(names):
                    wandb_logger.wandb.log({f'batch/cnt_{_name}':counts_dict.get(i,0), 'step/batch':ni})
                wandb_logger.wandb.log({'batch/loss':loss.item(), 'step/batch':ni})

                for _name, _loss in zip(['lbox', 'lobj', 'lcls', 'loss'], loss_items):
                    wandb_logger.wandb.log({f'batch/loss_{_name}':_loss.item(), 'step/batch':ni})
        ...
        if rank in [-1, 0]:
            ...
            if not opt.notest or final_epoch:  # Calculate mAP
                ...
                if wandb_logger.wandb:
                    for tag, score in zip(names, maps):
                        wandb_logger.wandb.log({"metrics/mAP_"+ tag: score, 'step/epoch': epoch})  # W&B
            # Write
            ...
            for x, tag in zip(list(mloss[:-1]) + list(results) + lr, tags):
                ...
                if wandb_logger.wandb:
                    wandb_logger.log({tag: x, 'step/epoch': epoch})  # W&B
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cut-paste', type=int, nargs='+', default=None, help='pass class list to enable cut paste \
                                                                                function. --cut-paste 1 2 3')
